...
for(j in 1:ncol(pXts)){
	dXts <- merge.xts(dXts, monthlyReturn(pXts[,j]))
}
...
Common.PlotCumReturns(dXts, "Sector Returns", "")

fullRets <- data.frame(apply(dXts, 2, Return.cumulative))
names(fullRets) <- c('R')
fullRets$N <- rownames(fullRets)

cumRetXts <- NULL
for(j in 1:ncol(dXts)){
	cumRetXts <- merge.xts(cumRetXts, rollapply(dXts[,j], 1, Return.cumulative))
}
names(cumRetXts) <- minMax$index_name
numCols <- ncol(cumRetXts)

allXts <- merge(Common.NormalizeMonthlyDates(cumRetXts), Common.NormalizeMonthlyDates(stats::lag(dXts, -1)), Common.NormalizeMonthlyDates(stats::lag(bench, -1)))
allXts <- na.omit(allXts)
maxCum <- data.frame(apply(allXts[, 1:numCols], 1, function(x) which(x == max(x))))
maxXts <- xts(maxCum, as.Date(rownames(maxCum)))
...
Common.PlotCumReturns(retXts, "Sector Rotation", "1/1")