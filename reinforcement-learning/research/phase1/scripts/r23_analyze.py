"""R2.3 consumer — MCX GOLD intraday arms vs baselines (per-day eval).

Arms: r23_u21, r23_sr. Baselines: intraday B&H, random, flat, daily B&H.
Monthly walk-forward over the short test window. Verdict: does intraday RL
find edge on an overnight-less venue (per-day Sharpe > 1, beats B&H/flat)?

Usage: python scripts/r23_analyze.py
"""
...
def main():
    ...
    print(f"GOLD26OCTFUT: evaluating {len(test_days)} test days "
          f"({test_days[0]} .. {test_days[-1]})")

    results = {}
    metas = {}
    for tag, lab in ARMS:
        ck = PHASE1 / "runs" / f"r23_{tag}" / "checkpoints"
    ...
    print("\n=== PER-DAY METRICS (MCX GOLD, test window, 5bps) ===")
    rows = []
    ...
    print("\n=== WALK-FORWARD (per-day Sharpe by month) ===")
    fold_rows = []
    ...
    plot_cum_drawdown(results, "R2.3 MCX GOLD intraday arms vs baselines "
    ...
    ax.set_title("R2.3 walk-forward: per-day Sharpe by month (MCX GOLD 1-min)")
    ...
    lines = [
        ...
        "",
        f"> GOLD26OCTFUT 1-min | lookback {LOOKBACK} | {int(COST*10000)}bps | "
        f"train {metas['U(2,1)']['train_start']}..{metas['U(2,1)']['train_end']} "
        f"(first 60% of days; no house windows — data starts 2025-11) | "
    ...
    if np.isfinite(m[best][0]) and m[best][0] > 1.0 and m[best][0] > bh_sr and m[best][0] > 0.0:
        ...
        lines.append(f"- Best arm {best} ({m[best][0]:+.2f}) is positive and beats flat — "
                     "promising on the overnight-less venue, but below the per-day Sharpe > 1 gate.")
    else:
        lines.append("- No arm beats flat even on the overnight-less venue — honest negative for R2.3.")
    lines += ["", "*Generated by r23_analyze.py — @StockViz*"]
