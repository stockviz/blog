...
for(ii in 2:nrow(rebalDates)){
	...
	pXts <- pXts[,toKeep]
	volvolDf <- data.frame(SYMBOL="", VOL=0.0)
	for(i in 1:ncol(pXts)){
		volvolDf <- rbind(volvolDf, c(toString(names(pXts)[i]), last(coredata(pXts[,i]))))
	}
	volvolDf <- volvolDf[-1,]
...
save(symRets, file=sprintf("%s/symRetsX1.Rdata", reportPath))

########################################

symXts <- xts(symRets[,-1], symRets[,1])

Common.PlotCumReturns(symXts, "Vol", "NIFTY 200 TR", sprintf("%s/sortX1.png", reportPath), NULL)
