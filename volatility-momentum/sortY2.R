### momentum x volatility (double sort)
...
for(ii in 2:nrow(rebalDates)){
	...
	for(i in 1:nrow(highRet)){
		...
		highRet$VOL[i] <- as.numeric(last(volatility(xts(pxDf[,-1], pxDf[, 1]), calc="parkinson")))
	}
...
save(symRets, file=sprintf("%s/symRetsY2.Rdata", reportPath))

########################################

symXts <- xts(symRets[,-1], symRets[,1])

Common.PlotCumReturns(symXts, "Momentum x Vol", "NIFTY 200 TR", sprintf("%s/sortY2.png", reportPath), NULL)
