### volatility and momentum (double sort)
...
for(ii in 2:nrow(rebalDates)){
	...
	lowVol <- head(volvolDf[order(volvolDf$VOL),], 50)
	...
	for(i in 1:nrow(lowVol)){
		...
		lowVol$RET[i] <- as.numeric(Return.cumulative(dailyReturn(xts(pxDf[,2], pxDf[,1]))))
	}
	
	lowVol <- head(lowVol[order(lowVol$RET, decreasing=T),], 25)
...
save(symRets, file=sprintf("%s/symRetsX2-A.Rdata", reportPath))

########################################

symXts <- xts(symRets[,-1], symRets[,1])

Common.PlotCumReturns(symXts, "Vol x Momentum", "NIFTY 200 TR", sprintf("%s/sortX2-A.png", reportPath), NULL)
