...
rebalFreqs <- c(1, 6) #months
fmtPeriods <- c(12) #months
portfolioSize <- seq(10, 100, by=10) #items
...
symRets <- data.frame(PERIOD_END="", STRATEGY="", REBAL_FREQ=0, FORMATION_PERIOD=0, SIZE=0, RET=0.0)
		...
		for(skipMo in skipMonths){
			...
			for(ii in (skipMo+2):nrow(rebalDates)){
				...
				if(freqCtr %% rfreq == 0){
					...
					highRet <- head(retDf[order(retDf$RET, decreasing=T),], 200)
				}
				freqCtr <- freqCtr + 1
				
				expectedTs <- nrow(indexXts[paste0(startDate, "/", endDate)])
				
				for(pSize in portfolioSize){
					periodRet <- getReturns(highRet$SYMBOL, startDate, endDate, expectedTs, pSize)
					symRets <- rbind(symRets, c(toString(endDate), paste0("MOM_", skipMo), rfreq, fmtPer, pSize, periodRet))
				}
			}
		}
	}
...
for(ii in 2:nrow(rebalDates)){
	...
	symRets <- rbind(symRets, c(toString(endDate), "BENCH", 0, 0, 0, benchRet))
}
