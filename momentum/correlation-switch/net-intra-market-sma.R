...
allXts$SMA_RET <- ifelse(allXts$BH > SMA(allXts$BH, smaLb), allXts$BH_RET, 0)
pflag <- ifelse(allXts$BH > SMA(allXts$BH, smaLb), 0, 1)
tflag <- pflag - stats::lag(pflag, -1)
allXts$SMA_RET <- allXts$SMA_RET - drag*abs(tflag)

allXts$COR_RET <- ifelse(allXts$ST_COR > allXts$LT_COR, allXts$BH_RET, 0)
pflag <- ifelse(allXts$ST_COR > allXts$LT_COR, 0, 1)
tflag <- pflag - stats::lag(pflag, -1)
allXts$COR_RET <- allXts$COR_RET - drag*abs(tflag)

allXts$COMBINED_RET <- ifelse(allXts$BH > SMA(allXts$BH) & allXts$ST_COR > allXts$LT_COR, allXts$BH_RET, 0)
pflag <- ifelse(allXts$BH > SMA(allXts$BH) & allXts$ST_COR > allXts$LT_COR, 0, 1)
tflag <- pflag - stats::lag(pflag, -1)
allXts$COMBINED_RET <- allXts$COMBINED_RET - drag*abs(tflag)

allXts$EITHER_RET <- ifelse(allXts$BH > SMA(allXts$BH) | allXts$ST_COR > allXts$LT_COR, allXts$BH_RET, 0)
pflag <- ifelse(allXts$BH > SMA(allXts$BH) | allXts$ST_COR > allXts$LT_COR, 0, 1)
tflag <- pflag - stats::lag(pflag, -1)
allXts$EITHER_RET <- allXts$EITHER_RET - drag*abs(tflag)

scenXts <- allXts[, c(c('COMBINED_RET', 'EITHER_RET', 'COR_RET', 'SMA_RET', 'BH_RET'))]
...
Common.PlotCumReturns(scenXts["/2019",], 
                      ...
                      sprintf("%s/momentum.correlation.sma.net.pre.png", reportPath))

sr <- SharpeRatio.annualized(scenXts["2020-05-01/",])
Common.PlotCumReturns(scenXts["2020-05-01/",], 
                      "Net Strategy Returns", 
                      sprintf("sr: %s", paste(round(sr, 2), collapse="/")), 
                      sprintf("%s/momentum.correlation.sma.net.post.png", reportPath))

sr <- SharpeRatio.annualized(scenXts)
Common.PlotCumReturns(scenXts, 
                      "Net Strategy Returns", 
                      sprintf("sr: %s", paste(round(sr, 2), collapse="/")), 
                      sprintf("%s/momentum.correlation.sma.net.png", reportPath))
