...
for(momLb in momLbs){
  ...
  for(i in 1:(length(monthEndDates)-1)){
    ...
    newNames <- (highMomNames |> filter(month_end == meDate) |> select(name))$name
...
save(momReturns, file=sprintf("%s/momentum.Rdata", reportPath))

toPlot <- na.omit(merge(momReturns, bench_monthly_rets))
sr <- paste(round(SharpeRatio.annualized(toPlot), 2), collapse = "/")
Common.PlotCumReturns(toPlot, "MSCI Country Index Momentum",
                      sprintf("%d/%d/EW | SR: %s", length(valid_countries), positionSize, sr),
                      sprintf("%s/msci-country-index-momentum.cumret.png", reportPath), NULL)
...
summary_tbl |>
  gt() |>
  tab_header(
    title = sprintf("MSCI Country Index Momentum (%d/%d/EW)", length(valid_countries), positionSize),
...
tbl |> gtsave(sprintf("%s/msci-momentum-summary.html", reportPath))

webshot2::webshot(
  sprintf("%s/msci-momentum-summary.html", reportPath),
  sprintf("%s/msci-momentum-summary.png", reportPath),
...
tbl <- ar_tbl |>
  gt() |>
  tab_header(
    title = sprintf("MSCI Country Index Momentum — Annual Returns (%d/%d/EW)", length(valid_countries), positionSize),
    subtitle = sprintf("%s → %s",
                       as.character(as.Date(first(index(toPlot)))),
                       as.character(as.Date(last(index(toPlot)))))
  ) |>
...
tbl |> gtsave(sprintf("%s/msci-momentum-annual.returns.html", reportPath))
webshot2::webshot(
  sprintf("%s/msci-momentum-annual.returns.html", reportPath),
  sprintf("%s/msci-momentum-annual.returns.table.png", reportPath),
  selector = "table.gt_table", expand = c(10, 10, 10, 10))
...
tbl <- dd_tbl |>
  gt() |>
  tab_header(
    title = sprintf("MSCI Country Index Momentum — Max Drawdown (%d/%d/EW)", length(valid_countries), positionSize),
    subtitle = sprintf("%s → %s",
                       as.character(as.Date(first(index(toPlot)))),
                       as.character(as.Date(last(index(toPlot)))))
  ) |>
...
tbl |> gtsave(sprintf("%s/msci-momentum-annual.drawdowns.html", reportPath))
webshot2::webshot(
  sprintf("%s/msci-momentum-annual.drawdowns.html", reportPath),
  sprintf("%s/msci-momentum-annual.drawdowns.table.png", reportPath),
  selector = "table.gt_table", expand = c(10, 10, 10, 10))
