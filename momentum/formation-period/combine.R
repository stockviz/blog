...
sharpe <- symRets %>% group_by(STRATEGY, REBAL_FREQ, FORMATION_PERIOD) %>% 
			tq_performance(RET, performance_fun=SharpeRatio.annualized)
			
annPerf <- symRets %>% group_by(STRATEGY, REBAL_FREQ, FORMATION_PERIOD) %>% 
			tq_performance(RET, performance_fun=Return.annualized)
			
statsDf <- sharpe %>% inner_join(annPerf) %>% rename(SHARPE = 4, RET = 5) %>% ungroup()
...
for(i in 1:nrow(configs)){
	tsDf <- symRets %>% filter(STRATEGY == configs$STRATEGY[i] & REBAL_FREQ == configs$REBAL_FREQ[i] & FORMATION_PERIOD == configs$FORMATION_PERIOD[i]) %>% select(RET, PERIOD_END)
...
names(toPlot) <- c(paste0(configs$STRATEGY, "_", configs$REBAL_FREQ, "_", configs$FORMATION_PERIOD), "BENCH")

Common.PlotCumReturns(toPlot, "Momentum", "skip-months/rebal-frequency/formation-period", sprintf("%s/symRetsAll.png", reportPath), NULL)
