# ============================================================================
# Single-Stock Winner / NIFTY-Short Momentum Backtest — Sharpe-Ratio Signal
# ============================================================================
# Same as backtest.R but ranks stocks by Sharpe ratio over the formation
# window instead of raw cumulative return. Vol-adjusted momentum signal.
#
# Output files suffixed with "_SR_" to differentiate from raw-return version.
# ============================================================================
...
# Sharpe-ratio momentum: annualized Sharpe of daily returns over formation window
momSR <- function(tkr, momEndNum, momStartNum) {
  r <- totalRetList[[tkr]]
  ...
  mu / sg * sqrt(252)  # annualized Sharpe
}
...
for (j in allJ) {
  ...
  for (mi in seq(warmupJ, length(monthEndsActual))) {
    ...
    momRets <- vapply(valid_tickers, momSR,
...
outputConfig(paperRes,  "paper_best_SR", PAPER_J, PAPER_K, PAPER_N)
outputConfig(searchRes, "search_SR",     SEARCH_J, SEARCH_K, SEARCH_N)
...
if (nrow(sweepResults) > 0) {
  ...
  gtsave(gtSweep, sprintf("%s/search_SR_sweep_metrics.html", reportPath))
  webshot2::webshot(
    paste0("file://", reportPath, "/search_SR_sweep_metrics.html"),
    sprintf("%s/search_SR_sweep_metrics.png", reportPath),
    selector = "table", expand = c(10, 10, 10, 10))
  cat("  search_SR_sweep_metrics.png\n")
}
