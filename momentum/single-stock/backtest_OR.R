# ============================================================================
# Single-Stock Winner / NIFTY-Short Momentum Backtest — Omega-Ratio Signal
# ============================================================================
# Same as backtest_SR.R but ranks stocks by Omega ratio over the formation
# window instead of Sharpe ratio. MAR = NIFTY daily return over same period.
# Beta-hedged short leg.
#
# Output files suffixed with "_OR_" to differentiate.
# ============================================================================
...
# Omega-ratio momentum: Omega(stock daily rets, MAR = avg NIFTY daily ret)
# Uses PerformanceAnalytics::Omega. Omega > 1 means stock beats NIFTY more often.
momOmega <- function(tkr, momEndNum, momStartNum) {
  r <- totalRetList[[tkr]]
  if (is.null(r)) return(NA_real_)
  di <- as.numeric(index(r))
  ie <- findInterval(momEndNum, di)
  is_val <- findInterval(momStartNum, di)
  if (is_val < 1L || ie < 1L || ie - is_val < 60L) return(NA_real_)
  rets <- as.numeric(coredata(r[is_val:ie, ]))
  if (length(rets) < 60) return(NA_real_)
  # NIFTY return over same window as daily MAR
  nfDi <- as.numeric(index(niftyIdxRets))
  nfIe <- findInterval(momEndNum, nfDi)
  nfIs <- findInterval(momStartNum, nfDi)
  if (nfIs < 1L || nfIe < 1L || nfIe - nfIs < 60L) return(NA_real_)
  nfRets <- as.numeric(coredata(niftyIdxRets[nfIs:nfIe, ]))
  dailyMAR <- mean(nfRets, na.rm = TRUE)
  if (is.na(dailyMAR)) return(NA_real_)
  tryCatch(Omega(rets, MAR = dailyMAR), error = function(e) NA_real_)
}
...
niftyIdxRets <- dailyReturn(niftyXts)  # needed by momOmega for MAR
...
for (j in allJ) {
  ...
  for (mi in seq(warmupJ, length(monthEndsActual))) {
    ...
    momRets <- vapply(valid_tickers, momOmega,
...
# Pre-compute 1-year trailing beta vs NIFTY for all tickers at all month-ends
cat("  Beta...\n")

betaCache <- vector("list", length(monthEndsActual))  # betaCache[[mi]][tkr] = beta
for (mi in seq_len(length(monthEndsActual))) {
  sigDate <- monthEndsActual[mi]

  # NIFTY 12-month window using xts date subsetting
  betaStart <- sigDate %m-% months(12)
  nfSub <- niftyIdxRets[paste0(as.character(betaStart), "/", as.character(sigDate))]
  if (nrow(nfSub) < 200) { betaCache[[mi]] <- numeric(0); next }

  betas <- vapply(valid_tickers, function(tkr) {
    r <- totalRetList[[tkr]]
    if (is.null(r)) return(NA_real_)
    # Extract same window, align on common dates
    stSub <- r[paste0(as.character(betaStart), "/", as.character(sigDate))]
    common <- na.omit(merge(stSub, nfSub))
    if (nrow(common) < 200L) return(NA_real_)
    stRets <- as.numeric(coredata(common[, 1L]))
    nfAligned <- as.numeric(coredata(common[, 2L]))
    cov(stRets, nfAligned) / var(nfAligned)
  }, FUN.VALUE = double(1))
  names(betas) <- valid_tickers
  betas <- betas[!is.na(betas)]
  betaCache[[mi]] <- betas
}
nBeta <- sum(sapply(betaCache, length) > 0)
cat(sprintf("    %d months cached\n", nBeta))
...
runBacktest <- function(J, K, N, trainEnd = NULL, quiet = FALSE, fast = FALSE, monthlyOnly = FALSE) {
  ...
  if (monthlyOnly) {
    ...
    for (mi in seq(firstIdx, endIdx)) {
      ...
      for (tkr in topStocks) {
        ...
        if (length(cd) < 5) next
        # Beta-hedged, capital-neutral: (stock - beta*nifty) / (1+|beta|)
        betaVal <- betaCache[[mi]][tkr]
        if (is.na(betaVal) || is.null(betaVal)) betaVal <- 1.0
        scale <- 1.0 / (1.0 + abs(betaVal))
        combined <- (as.numeric(coredata(stockSub[cd])) - betaVal * as.numeric(coredata(niftySub[cd]))) * scale
        combined[1] <- combined[1] - DRAG * scale
  ...
  for (mi in seq(firstIdx, endIdx)) {
    ...
    for (tkr in topStocks) {
      ...
      if (is.na(betaVal) || is.null(betaVal)) betaVal <- 1.0
      scale <- 1.0 / (1.0 + abs(betaVal))
      strandCombined <- (as.numeric(coredata(stockSub[cd])) -
                        betaVal * as.numeric(coredata(niftySub[cd]))) * scale
      strandCombined[1] <- strandCombined[1] - DRAG * scale
...
outputConfig(paperRes,  "paper_best_OR", PAPER_J, PAPER_K, PAPER_N)
outputConfig(searchRes, "search_OR",     SEARCH_J, SEARCH_K, SEARCH_N)
...
if (nrow(sweepResults) > 0) {
  ...
  gtsave(gtSweep, sprintf("%s/search_OR_sweep_metrics.html", reportPath))
  webshot2::webshot(
    paste0("file://", reportPath, "/search_OR_sweep_metrics.html"),
    sprintf("%s/search_OR_sweep_metrics.png", reportPath),
    selector = "table", expand = c(10, 10, 10, 10))
  cat("  search_OR_sweep_metrics.png\n")
}
