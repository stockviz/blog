...
# ── gt table ──
SCENARIO_COLORS <- c(`1-Week` = "#f0fff0", `2-Week` = "#f0f4ff", `4-Week` = "#fff3e0")
BENCH_COLOR <- "#f5f5f5"
...
perfTbl <- tibble(
  ...
  gt() |>
  tab_header(
    title = "Industry Rotation — RRG Top 5 (Improving)",
...
gtsave(perfTbl, sprintf("%s/improving-rrg.html", reportPath))
webshot2::webshot(sprintf("%s/improving-rrg.html", reportPath),
                  sprintf("%s/improving-rrg-tbl.png", reportPath),
...
Common.PlotCumReturns(plotData,
  "Industry Rotation — RRG Top 5 (Improving)",
  sprintf("rsRatio < 100 & rsMomentum > 100; best: %s, %s | Sharpe: %s",
          bestCap, bestEq, srText),
  sprintf("%s/improving-rrg.png", reportPath), NULL)
