source("/mnt/data/blog/industry-rotation/improving/common.R")

# ═══════════════════════════════════════════════════════════════════════════
#  Staggered 4-week: Improving quadrant (RS-Ratio < 100, RS-Mom > 100)
# ═══════════════════════════════════════════════════════════════════════════
...
perfTbl <- tibble(
  ...
  gt() |>
  tab_header(
    title = "Industry Rotation — RRG Top 5 (Improving, Staggered 4W)",
    subtitle = sprintf("rsRatio < 100 & rsMomentum > 100; %.0f bps drag; avg of %d scenarios; %s to %s",
...
gtsave(perfTbl, sprintf("%s/improving-4w.html", reportPath))
webshot2::webshot(sprintf("%s/improving-4w.html", reportPath),
                  sprintf("%s/improving-4w-tbl.png", reportPath),
                  selector = "table.gt_table", expand = c(10, 10, 10, 10))

# ── plot (averages + benchmarks) ──
...
Common.PlotCumReturns(plotData,
  "Industry Rotation — RRG Top 5 (Improving, Staggered 4W)",
  sprintf("rsRatio < 100 & rsMomentum > 100; avg of %d scenarios | Sharpe: %s",
          N_SCENARIOS, srText),
  sprintf("%s/improving-4w.png", reportPath), NULL)
