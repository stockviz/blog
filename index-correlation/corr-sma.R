...
for(corLb in corLbs){
	...
	allXts$SMA_COR_RET <- ifelse(allXts$INDEX > allXts$INDEX_SMA & allXts[, corName] < tileInspect, allXts$RET_1, 0)
	allXts$COR_RET <- ifelse(allXts[, corName] < tileInspect, allXts$RET_1, 0)
	
	Common.PlotCumReturns(allXts[, c('SMA_RET', 'COR_RET', 'SMA_COR_RET', 'RET_1')], 
