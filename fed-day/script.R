...
toPlot <- merge(indexXts, fomcXts$DISC_RATE)
...
write.csv(fomcEffectDf, file=sprintf("%s/fomcEffect.returns.csv", reportPath))
...
for(lb in lbs){
	...
	p1 + p2 + p3 + plot_layout(ncol=1) + 
				plot_annotation(title = sprintf("%d-day %s Returns on FOMC Dates", lb, indexName), 
					subtitle = sprintf("[%s:%s]", first(index(fomcAnal)), last(index(fomcAnal))),
					caption='@StockViz') 

	ggsave(sprintf("%s/%s-index.returns.%d.png", reportPath, indexName, lb), width=12, height=12*length(lbs), units="in")					
}
...
for(lb in lbs){
	...
	ggsave(sprintf("%s/%s-index.returns.pre-post.%d.png", reportPath, indexName, lb), width=12, height=12*length(lbs), units="in")					
}			
...
write.csv(fomcEffectDf, file=sprintf("%s/fomcEffect.sd.csv", reportPath))
...
for(lb in lbs){
	...
	ggsave(sprintf("%s/%s-index.sd.%d.png", reportPath, indexName, lb), width=12, height=24, units="in")					
}
...
for(lb in lbs){
	...
	ggsave(sprintf("%s/%s-index.sd.pre-post.%d.png", reportPath, indexName, lb), width=12, height=12*length(lbs), units="in")					
}			
