...
This document explores the practical application of physics-based market turbulence concepts to financial markets using exploratory tests on NIFTY futures, synthetic SELECT futures series, and specific index data. The analysis investigates the implementation of a source paper by Zheng and Dong regarding Quantum Temporal Winds, detailing methodology choices such as Bézier curve fitting, turning-point detectors, turbulence thresholds, and trading costs. The research compares various fixed-parameter interpretations, including reversal and breakout strategies, along with trend-filtered variants using moving averages. Results indicate that while the approach shows potential in certain regimes, it struggles in trending markets, suffers from parameter overfitting, and faces challenges with walk-forward testing. The source material provides comprehensive code scripts, performance metrics, and configuration details for further quantitative finance research.
