...
This article details a rigorous research program applying reinforcement learning (RL) to Indian stock and futures markets using the StockViz data estate. The study tested various RL agents across multiple rewards, costs, states, actions, algorithms, and venues, including both daily and intraday frequencies. The primary finding is a documented negative result: no RL agent outperformed classical baselines, such as naive momentum and trend strategies, net of transaction costs out-of-sample. The research also uncovered significant market-structure insights, notably that NIFTY-family intraday trading is negative-carry, with returns primarily driven by overnight gaps rather than daytime price action. The program cataloged specific RL failure modes, including drift-betting, flat-optimality, and churn, providing a reproducible pipeline and taxonomy for future specialist implementation attempts.
