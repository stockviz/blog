...
Common.PlotCumReturns<-function(toPlot, chartTitle, chartSubTitle, fileName=NULL, dest=NULL){
	...
	tryCatch({
		...
		mtext(chartSubTitle, cex=0.9, line=0.5)
		mtext(sprintf("cumulative: %s\nannualized: %s", paste(sprintf("%.2f%%", 100*apply(toPlot, 2, Return.cumulative)), collapse=" / "), paste(sprintf("%.2f%%", 100*apply(toPlot, 2, Return.annualized)), collapse=" / ")), cex=1, line=-1.5)
		par(mar = c(5, 4, 0, 2))
