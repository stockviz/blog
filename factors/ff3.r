...
toPlot <- merge(smbXts, hmlXts)
names(toPlot) <- c('SMB', 'HML')

Common.PlotCumReturns(toPlot, "Fama-French Factors", "", sprintf("%s/fama-french-smb-hml.png", reportPath), NULL)

cmTr <- merge(cumprod(1 +mktXts), cumprod(1 +smbXts), cumprod(1 +hmlXts))

annRets <- merge(yearlyReturn(cmTr[,1]), yearlyReturn(cmTr[,2]), yearlyReturn(cmTr[,3]))
names(annRets) <- c('MKT', 'SMB', 'HML')
...
ggsave(sprintf("%s/fama-french-smb-hml.annual.png", reportPath), width=16, height=8, units="in")