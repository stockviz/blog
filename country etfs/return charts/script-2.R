...
tt1<-arrangeGrob(grobs=list(tableGrob(indices[, c('CODE', 'NAME')], rows=NULL, theme=tableTheme)), ncol=1, top=textGrob("NASDAQ Country TR Indices", gp=gpar(fontsize=14, fontface ='bold', fontfamily='Segoe UI')), bottom=textGrob("@StockViz", gp=gpar(fontsize=10, col='grey', fontfamily='Segoe UI')))

ggsave(sprintf("%s/INDEX-NQ.list.%s.%s.png", reportPath, startDate, endDate), tt1, width=6, height=nrow(indices)*0.3, units='in')	
