...
reportPath <- "D:/StockViz/public/blog/beta-hamming"
library('RODBC')
library('RPostgres')
library('tidyverse')
library('reshape2')
...
rollBeta <- merge(rollapply(retXts, segLength, function(X) CAPM.beta(X[,1], X[, 2]), by.column = F), rollapply(retXts, segLength, function(X) CAPM.beta(X[,1], X[, 3]), by.column = F))
...
ggplot(toPlot, aes(x=T, y=value, color=variable)) +
	...
	labs(x='', y='VIX', color='', title=sprintf("%s Beta", ticker), subtitle=sprintf("%d-day [%s:%s]", segLength, first(index(retXts)), last(index(retXts)))) +
...
ggsave(sprintf("%s/%s-beta.png", reportPath, ticker), width=16, height=8, units="in")
