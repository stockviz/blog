...
#model <- c('1A6C40B8-BDF1-43E5-829C-E3265BDB7F1A', 'D6663194-A965-4262-84F6-BB553B0B8998', 'Momentum')
#model <- c('AFD0DFFF-2EA7-4E4D-BA50-D9CC0E4B5052', '6CAECA7A-A2C9-4B9D-BD0B-397FCE9597D2', 'Velocity')
model <- c('22849AAA-554B-4859-B06E-760B1902104B', 'A251E57D-B2B2-43CA-A457-E25C2CAD08A9', 'Acceleration')
...
Common.PlotCumReturns(testRets, sprintf("Momo %s", model[3]), "(EOD rebalance)", sprintf("%s/momo.%s.test.cumulative.png", reportPath, model[1]))
...
names(valRets) <- c(model[3], maxRetSMA, minDDSMA, paste(model[3], '(fixed)'))

Common.PlotCumReturns(valRets, sprintf("Momo %s", model[3]), "(EOD rebalance)", sprintf("%s/momo.%s.validate.cumulative.png", reportPath, model[1]))
...
names(insRets) <- c(model[3], maxRetSMA, minDDSMA, paste(model[3], '(fixed)'))

Common.PlotCumReturns(insRets, sprintf("Momo %s", model[3]), "(EOD rebalance)", sprintf("%s/momo.%s.inspect.cumulative.png", reportPath, model[1]))
