...
indexNames <- c("SPY", "QQQ")

indexPx <- NULL
for(iName in indexNames){
	iDf <- sqlQuery(lconUs2, sprintf("select C, time_stamp from BHAV_EQ_TD where SYMBOL = '%s'", iName))
...
for(j in 1:ncol(indexPx)){
	...
	### time between ATH
	kable(inrAthDatesTb, caption = sprintf('%s Days between All Time Highs', indexNames[j]), booktabs = T) %>% 
			kable_styling(bootstrap_options = 'striped', full_width = F) %>% 
			save_kable(file=sprintf("%s/%s-ath-drawdowns.png", reportPath, indexNames[j]), bs_theme = 'flatly')
	...
	ggplot(toPlot, aes(x=T, y=.data[[jName]])) +
		theme_economist() +
	...
	ggsave(sprintf("%s/%s-ath-price.png", reportPath, indexNames[j]), width=12, height=6, units="in")
