...
usdIndex<-args[1] #DTWEXB, DTWEXO, DEXINUS
startYear<-as.numeric(args[2]) #2000, 2005

reportPath <- sprintf("./%s/%s", usdIndex, startYear)
dir.create(file.path(reportPath), recursive=T)
...
executeKernal<-function(ds, validationSet, suffix=""){
	...
	ggplot(svecModel.ape, aes(x=ACTUAL,y=APE)) + 
		theme_economist() +
		geom_point() +
		labs(x = "actual", y="ape", fill="", color="", title=sprintf("SVM %s/NIFTY (%d)", usdIndex, ds), subtitle=sprintf("Absolute Percent Error [%s:%s]", plotStart, plotEnd)) +
	...
	Common.PlotCumReturns(retCumXts, sprintf("SVM %s/NIFTY (%d)", usdIndex, ds), sprintf("%s/%s.NIFTY.%d.cumulative.%s.png", reportPath, usdIndex, ds, suffix))
...
startDate<-as.Date(sprintf("%d-01-01", startYear))
