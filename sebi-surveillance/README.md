...
For readability, the four event charts remove observations more than three standard deviations from the mean within each measure/event-type group. This trimming affects chart display only: the CSV summaries, p-values, RDS bundle, and reported findings retain the complete usable sample. The chart subtitles identify the trimmed sample size.
