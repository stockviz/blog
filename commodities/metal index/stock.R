...
mcxIndexName <- "MCXALUMDEX"
nseIndexName <- "NATIONALUM"
nseMktName <- "NIFTY 50"
...
nPx <- dbGetQuery(pgCon, "select date_stamp, o, c from eod_adjusted_nse where ticker=$1 and date_stamp >= $2", params=list(nseIndexName, startDate))
...
ebitDf <- sqlQuery(lcon, sprintf("select PERIOD_END, max(item_val) EBIT from FUNDA_XBRL 
							where SYMBOL='%s' and PERIOD_TYPE='QUARTERLY' and ITEM_KEY='EBIT' and IS_CONSOLIDATED=1 
							group by PERIOD_END 
							order by PERIOD_END", nseIndexName))
		
ebitDf$EBIT_SQRT <- sign(ebitDf$EBIT) * sqrt(abs(ebitDf$EBIT))
ggplot(ebitDf, aes(x=PERIOD_END, y=EBIT_SQRT)) +
	theme_economist() +
	theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
	geom_bar(stat="identity", position=position_dodge(), fill = (viridis_pal()(2))[1]) +
	scale_x_date(breaks = "6 months", date_labels="%b-%Y") +
	labs(x = "", y="EBIT sqrt(Rs. '000)", title=sprintf("%s Quarterly EBIT", nseIndexName), subtitle=sprintf("%s:%s; source: NSE", min(ebitDf$PERIOD_END), max(ebitDf$PERIOD_END))) 
ggsave(sprintf("%s/%s.EBIT.png", reportPath, nseIndexName), width=12, height=6, units="in")	
