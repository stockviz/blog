...
lookbacks_for_regime <- function(regime,
                                 regime_lookbacks = c(Green = 10L, Yellow = 3L, Red = 1L)) {
  required_regimes <- c("Green", "Yellow", "Red")
  if (!identical(sort(names(regime_lookbacks)), sort(required_regimes))) {
    stop("regime_lookbacks must name Green, Yellow, and Red")
  }
  regime_lookbacks <- as.integer(regime_lookbacks[required_regimes])
  names(regime_lookbacks) <- required_regimes
  if (any(regime_lookbacks < 1L)) stop("Regime lookbacks must be positive integers")
  unname(regime_lookbacks[regime])
}
...
apply_month_weights <- function(month_returns, weights, old_weights, cost_rate = 0) {
  missing_assets <- setdiff(names(weights), colnames(month_returns))
  ...
  gross_values <- as.numeric(as.matrix(month_returns[, names(weights), drop = FALSE]) %*% weights)
...
run_cross_portfolio <- function(cache, adaptive = TRUE, top_n = 1L, cost_rate = 0.0025,
  ...
  if (adaptive) {
    signal_data$lookback <- lookbacks_for_regime(signal_data$regime, regime_lookbacks)
    regime_lookbacks <- setNames(
      lookbacks_for_regime(c("Green", "Yellow", "Red"), regime_lookbacks),
      c("Green", "Yellow", "Red")
    )
    for (lb in unique(regime_lookbacks)) {
      nm <- as.character(lb)
      if (is.null(signal_data$momentum[[nm]])) {
        signal_data$momentum[[nm]] <- month_momentum(signal_data$levels, lb)
      }
    }
  }
  ...
  strategy <- if (!is.null(strategy_label)) {
    sprintf("%s Top %d", strategy_label, top_n)
  } else {
    sprintf("%s Top %d", if (adaptive) "VIX" else "10M", top_n)
  }
  ...
  for (i in seq_along(signal_data$dates)) {
    ...
    weights <- weights_from_momentum(scores, top_n, asset_order)
...
run_timing_portfolio <- function(cache, asset, mode = c("adaptive", "fixed", "buyhold"),
                                 cost_rate = 0.0025) {
  mode <- match.arg(mode)
  signal_data <- build_signal_data(cache)
  all_returns <- merge(cache$index_returns[, asset], cache$cash_returns, join = "inner")
  colnames(all_returns) <- c(asset, "CASH")
  all_returns <- na.omit(all_returns)
  strategy <- paste(asset, c(adaptive = "VIX", fixed = "10M", buyhold = "BuyHold")[[mode]], sep = " - ")
  old_weights <- c(setNames(0, asset), CASH = 1)
  daily_parts <- list()
  audit_parts <- list()
  out_i <- 0L
  ...
  for (i in seq_along(signal_data$dates)) {
    ...
    holding_dates <- holding_dates_after_signal(signal_date, as.Date(index(all_returns)))
    if (length(holding_dates) == 0L) next
    lookback <- if (mode == "adaptive") signal_data$lookback[i] else 10L
    if (is.na(lookback) || i <= lookback) next
    score <- as.numeric(signal_data$momentum[[as.character(lookback)]][i, asset])
    if (!is.finite(score)) next
    weights <- c(setNames(if (mode == "buyhold" || score >= 0) 1 else 0, asset), CASH = 0)
    weights["CASH"] <- 1 - weights[asset]
    month_returns <- all_returns[as.character(holding_dates)]
    pnl <- apply_month_weights(month_returns, weights, old_weights, cost_rate)
    ...
    row$asset <- asset
    row$momentum <- score
    row$weight_asset <- weights[asset]
    row$weight_CASH <- weights["CASH"]
    row$selected_assets <- if (weights[asset] > 0) asset else "CASH"
    row$turnover <- pnl$turnover
    row$gross_return <- prod(1 + as.numeric(pnl$gross)) - 1
    row$cost <- pnl$cost
    row$net_return <- prod(1 + as.numeric(pnl$net)) - 1
  ...
  stopifnot(!anyDuplicated(audit$signal_date))
  stopifnot(all(is.finite(as.numeric(daily))))
  stopifnot(all(abs(audit$weight_asset + audit$weight_CASH - 1) < 1e-12))
  list(daily = daily, audit = audit, strategy = strategy, cost_rate = cost_rate)
}
...
run_equal_weight <- function(cache, start_date, end_date) {
  ...
  for (month in unique(months)) {
    ...
    for (i in positions) {
      day_returns <- as.numeric(x[i, ])
      values[i] <- sum(weights * day_returns)
...
# Fixed-lookback momentum portfolio: rank assets on the trailing `lookback`-month
# return at each month-end, hold Top N for the following month. Unlike
# run_cross_portfolio, this does not use India VIX — the lookback is held fixed so
# the sensitivity of results to the lookback choice can be measured in isolation.
run_fixed_lookback_portfolio <- function(cache, top_n = 1L, lookback = 10L,
                                         cost_rate = 0.0025) {
  levels <- cache$month_ends
  signal_dates <- as.Date(index(levels))
  all_returns <- merge(cache$index_returns, cache$cash_returns, join = "inner")
  ...
  all_returns <- na.omit(all_returns)
  asset_order <- colnames(cache$month_ends)
  strategy <- sprintf("L%d Top %d", lookback, top_n)
  mom <- month_momentum(levels, lookback)
  old_weights <- setNames(rep(0, length(asset_order)), asset_order)
  old_weights["CASH"] <- 1
  daily_parts <- list()
  audit_parts <- list()
