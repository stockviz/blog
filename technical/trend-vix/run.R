...
run_all <- function() {
  ...
  for (cost_name in names(COST_LEVELS)) {
    ...
    timing[[cost_key]] <- list()
    for (asset in INDEX_NAMES) {
      asset_key <- make.names(asset)
      timing[[cost_key]][[asset_key]] <- list()
      for (mode in c("adaptive", "fixed", "buyhold")) {
        result <- run_timing_portfolio(cache, asset, mode, rate)
        timing[[cost_key]][[asset_key]][[mode]] <- result
        audit_i <- audit_i + 1L
        all_audits[[audit_i]] <- result$audit
      }
    }
  ...
  primary_results <- c(cross[["25bps"]],
                       unlist(timing[["25bps"]], recursive = FALSE, use.names = TRUE))
  primary_flat <- list()
  for (nm in names(cross[["25bps"]])) primary_flat[[paste0("Cross_", nm)]] <- cross[["25bps"]][[nm]]
  for (asset_key in names(timing[["25bps"]])) {
    for (mode in names(timing[["25bps"]][[asset_key]])) {
      primary_flat[[paste(asset_key, mode, sep = "_")]] <- timing[["25bps"]][[asset_key]][[mode]]
    }
  }
  primary_daily <- merge_daily_results(primary_flat)
