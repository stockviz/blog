...
if (nrow(results) > 0) {
  results |>
    ...
    gtsave(sprintf("%s/combined-summary-involwt-lo.html", reportPath))

  webshot2::webshot(
    sprintf("%s/combined-summary-involwt-lo.html", reportPath),
    sprintf("%s/combined-summary-involwt-lo.png", reportPath),
...
for (asset in names(scaled_lo)) {
  ...
  Common.PlotCumReturns(combined, asset,
    ...
    sprintf("%s/%s-involwt-lo.cumret.png", reportPath, safe_name), NULL)
}
...
if (length(valid_bin) > 1) {
  ...
  Common.PlotCumReturns(bin_all, "All Assets",
    ...
    sprintf("%s/combined-all-involwt.binary-lo.cumret.png", reportPath), NULL)
  ...
  Common.PlotCumReturns(sclo_all, "All Assets",
    ...
    sprintf("%s/combined-all-involwt.scaled-lo.cumret.png", reportPath), NULL)
}
...
for (group_name in c("NIFTY", "Crypto", "MCX", "US")) {
  ...
  Common.PlotCumReturns(g_sclo, group_name,
    ...
    sprintf("%s/group-involwt-%s.scaled-lo.cumret.png", reportPath, tolower(group_name)), NULL)
  ...
  Common.PlotCumReturns(g_bin, group_name,
    ...
    sprintf("%s/group-involwt-%s.binary-lo.cumret.png", reportPath, tolower(group_name)), NULL)
}
...
if (length(valid_bin) >= 2) {
  ...
  cat(sprintf("  Scaled LO  InvVol: SR=%.2f  Ret=%.2f%%  DD=%.2f%%\n",
              port_sr, port_ret*100, port_dd*100))
  cat(sprintf("  Binary LO  InvVol: SR=%.2f  Ret=%.2f%%  DD=%.2f%%\n",
  ...
  write.csv(port_df, sprintf("%s/portfolio-returns-involwt-lo.csv", reportPath), row.names = FALSE)
  cat(sprintf("  saved portfolio returns: portfolio-returns-involwt-lo.csv (%d rows)\n", nrow(port_df)))

  # ── gt portfolio table ──
  port_results <- tibble(
    Strategy   = c("Scaled LO (InvVol)", "Binary LO (InvVol)", "B&H (InvVol)"),
  ...
  port_results |>
    ...
    tab_source_note(source_note = "@StockViz") |>
    tab_style(style = cell_text(align = "right"), locations = cells_source_notes()) |>
    gtsave(sprintf("%s/portfolio-metrics-involwt-lo.html", reportPath))

  webshot2::webshot(
    sprintf("%s/portfolio-metrics-involwt-lo.html", reportPath),
    sprintf("%s/portfolio-metrics-involwt-lo.png", reportPath),
    selector = "table.gt_table", expand = c(10, 10, 10, 10))

  # ── cumulative chart: Scaled LO vs Binary LO vs B&H ──
  combined <- na.omit(merge(port_scaled, port_bin, port_bh))
  names(combined) <- c("Scaled LO (InvVol)", "Binary LO (InvVol)", "B&H (InvVol)")
  Common.PlotCumReturns(combined, sprintf("Inverse-Vol Portfolio (%d assets)", n_assets),
    ...
    sprintf("%s/portfolio-ew-involwt-lo.cumret.png", reportPath), NULL)
  ...
  tbl |> gtsave(sprintf("%s/portfolio-annual-involwt.returns-lo.html", reportPath))
  webshot2::webshot(
    sprintf("%s/portfolio-annual-involwt.returns-lo.html", reportPath),
    sprintf("%s/portfolio-annual-involwt.returns-lo.table.png", reportPath),
    selector = "table.gt_table", expand = c(10, 10, 10, 10))
  ...
  tbl |> gtsave(sprintf("%s/portfolio-annual-involwt.drawdowns-lo.html", reportPath))
  webshot2::webshot(
    sprintf("%s/portfolio-annual-involwt.drawdowns-lo.html", reportPath),
    sprintf("%s/portfolio-annual-involwt.drawdowns-lo.table.png", reportPath),
    selector = "table.gt_table", expand = c(10, 10, 10, 10))
  ...
  tbl |> gtsave(sprintf("%s/portfolio-annual-involwt.sharpe-lo.html", reportPath))
  webshot2::webshot(
    sprintf("%s/portfolio-annual-involwt.sharpe-lo.html", reportPath),
    sprintf("%s/portfolio-annual-involwt.sharpe-lo.table.png", reportPath),
    selector = "table.gt_table", expand = c(10, 10, 10, 10))
}
