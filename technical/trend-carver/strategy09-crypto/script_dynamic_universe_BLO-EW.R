...
for (i in seq_len(length(all_month_ends) - 1)) {
  ...
  for (sym in current_coins) {
    ...
    blo_xts <- xts(blo, order.by = common_dates)

    # evaluate Sharpe on last sharpe_lookback days only
    recent_blo <- tail(blo_xts, sharpe_lookback)
    if (nrow(recent_blo) < min_sharpe_rows) next
    sr <- tryCatch({
      if (sd(coredata(recent_blo), na.rm = TRUE) == 0) NA
      else SharpeRatio.annualized(recent_blo)[1,1]
    }, error = function(e) NA)
...
# ── save selection log ────────────────────────────────────────────────────────
write.csv(selection_log, sprintf("%s/dynamic-universe-selection-blo-ew.csv", reportPath), row.names = FALSE)
...
if (nrow(portfolio_rets) > 0) {
  ...
  cat(sprintf("  Dynamic BLO EW: SR=%.2f  Ret=%.2f%%  DD=%.2f%%\n", sr, ret*100, dd*100))
  ...
  write.csv(data.frame(Date = index(portfolio_rets), Return = coredata(portfolio_rets)),
            sprintf("%s/dynamic-universe-returns-blo-ew.csv", reportPath), row.names = FALSE)
  cat("  returns saved to dynamic-universe-returns-blo-ew.csv\n")
  ...
  if (!is.null(daily_prices[["BTCUSDT"]])) {
    ...
    names(to_plot) <- c("Dynamic BLO EW", "BTCUSDT B&H")

    dyn_sr <- tryCatch(SharpeRatio.annualized(to_plot[, "Dynamic BLO EW"])[1,1],
                        error = function(e) NA)

    Common.PlotCumReturns(to_plot, "Dynamic Universe — Binary Long-Only EW",
      sprintf("Monthly re-selection (≥%d bars, SR>0); SR = %.2f", min_daily_bars, dyn_sr),
      sprintf("%s/dynamic-universe-blo-ew.cumret.png", reportPath), NULL)
  }
}
