...
# ── save selection log ────────────────────────────────────────────────────────
write.csv(selection_log, sprintf("%s/dynamic-universe-selection-bls-ew-h.csv", reportPath), row.names = FALSE)
...
if (nrow(portfolio_rets) > 0) {
  ...
  cat(sprintf("  Dynamic BLS EW (H): SR=%.2f  Ret=%.2f%%  DD=%.2f%%\n", sr, ret*100, dd*100))
  ...
  write.csv(data.frame(Date = index(portfolio_rets), Return = coredata(portfolio_rets)),
            sprintf("%s/dynamic-universe-returns-bls-ew-h.csv", reportPath), row.names = FALSE)
  cat("  returns saved to dynamic-universe-returns-bls-ew-h.csv\n")
  ...
  if (!is.null(daily_prices[["BTCUSDT"]])) {
    ...
    names(to_plot) <- c("Dynamic BLS EW (H)", "BTCUSDT B&H")

    dyn_sr <- tryCatch(SharpeRatio.annualized(to_plot[, "Dynamic BLS EW (H)"])[1,1],
                        error = function(e) NA)

    Common.PlotCumReturns(to_plot, "Dynamic Universe — Binary Long-Short (Hurst) EW (Hurst)",
      sprintf("Monthly re-selection (≥%d bars, H>0.55); SR = %.2f", min_daily_bars, dyn_sr),
      sprintf("%s/dynamic-universe-bls-ew-h.cumret.png", reportPath), NULL)
  }
}
