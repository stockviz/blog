...
source("/mnt/data/blog/technical/trend-carver/strategy09/strategy_nine.R")
...
for (asset in all_assets) {
  ...
  # binary long-only
  strat_bin <- ifelse(coredata(sa) == 1, coredata(ra), 0)
  ...
  binary_lo[[asset]] <- bin_xts

  # scaled long-only
  w_lo <- pmax(pmin(coredata(fc), 1), 0)
  strat_sclo <- w_lo * coredata(ra)
  strat_sclo <- ifelse(trd != 0 & !is.na(trd), strat_sclo - drag, strat_sclo)
  scaled_lo[[asset]] <- xts(strat_sclo, order.by = common_dates)
  ...
  sclo_sr  <- SharpeRatio.annualized(scaled_lo[[asset]])[1,1]
  sclo_ret <- as.numeric(Return.annualized(scaled_lo[[asset]]))
  sclo_dd  <- as.numeric(maxDrawdown(scaled_lo[[asset]]))
...
if (nrow(results) > 0) {
  results |>
    ...
    gtsave(sprintf("%s/combined-summary-lo.html", reportPath))

  webshot2::webshot(
    sprintf("%s/combined-summary-lo.html", reportPath),
    sprintf("%s/combined-summary-lo.png", reportPath),
    selector = "table.gt_table", expand = c(10, 10, 10, 10))
}

# ── cumulative return charts per asset ───────────────────────────────────────
cat("\n=== Cumulative Charts ===\n")
for (asset in names(scaled_lo)) {
  combined <- na.omit(merge(scaled_lo[[asset]], binary_lo[[asset]], bh_rets[[asset]]))
  ...
  Common.PlotCumReturns(combined, asset,
    ...
    sprintf("%s/%s-lo.cumret.png", reportPath, safe_name), NULL)
}

# ── combined charts: all binary LO, all scaled LO ───────────────────────────
valid_bin <- names(binary_lo)
if (length(valid_bin) > 1) {
  bin_all <- do.call(merge.xts, lapply(valid_bin, \(a) binary_lo[[a]]))
  names(bin_all) <- valid_bin
  Common.PlotCumReturns(bin_all, "All Assets",
    "Strategy Nine Binary Long-Only — Equal Weight",
    sprintf("%s/combined-all-binary-lo.cumret.png", reportPath), NULL)

  sclo_all <- do.call(merge.xts, lapply(valid_bin, \(a) scaled_lo[[a]]))
  names(sclo_all) <- valid_bin
  Common.PlotCumReturns(sclo_all, "All Assets",
    "Strategy Nine Scaled Long-Only — Equal Weight",
    sprintf("%s/combined-all-scaled-lo.cumret.png", reportPath), NULL)
}
...
for (group_name in c("NIFTY", "Crypto", "MCX", "US")) {
  ...
  # scaled LO
  g_sclo <- do.call(merge.xts, lapply(group_assets, \(a) scaled_lo[[a]]))
  names(g_sclo) <- group_assets
  Common.PlotCumReturns(g_sclo, group_name,
    sprintf("Strategy Nine Scaled LO — %s (equal weight)", group_name),
    sprintf("%s/group-%s.scaled-lo.cumret.png", reportPath, tolower(group_name)), NULL)

  # binary LO
  g_bin <- do.call(merge.xts, lapply(group_assets, \(a) binary_lo[[a]]))
  names(g_bin) <- group_assets
  Common.PlotCumReturns(g_bin, group_name,
    sprintf("Strategy Nine Binary LO — %s (equal weight)", group_name),
    sprintf("%s/group-%s.binary-lo.cumret.png", reportPath, tolower(group_name)), NULL)
}

# ── Equal-Weight Portfolio: from date where ALL assets are available ────────── ────────────────────────────
cat("\n=== Equal-Weight Portfolio (common start) ===
\n")
if (length(valid_bin) >= 2) {

  # merge all scaled LO strategy returns
  ew_scaled <- do.call(merge.xts, lapply(valid_bin, \(a) scaled_lo[[a]]))
  ew_bin    <- do.call(merge.xts, lapply(valid_bin, \(a) binary_lo[[a]]))
  ...
  first_valid <- sapply(valid_bin, \(a) first(index(na.omit(scaled_lo[[a]]))))
  ...
  port_scaled <- xts(rowMeans(s_ret, na.rm = TRUE), order.by = index(s_ret))
  port_bin    <- xts(rowMeans(b_ret, na.rm = TRUE), order.by = index(b_ret))
  port_bh     <- xts(rowMeans(h_ret, na.rm = TRUE), order.by = index(h_ret))
  ...
  cat(sprintf("  Scaled LO  EW: SR=%.2f  Ret=%.2f%%  DD=%.2f%%\n",
              port_sr, port_ret*100, port_dd*100))
  cat(sprintf("  Binary LO  EW: SR=%.2f  Ret=%.2f%%  DD=%.2f%%\n",
              port_bin_sr, port_bin_ret*100, port_bin_dd*100))
  cat(sprintf("  B&H         EW: SR=%.2f  Ret=%.2f%%  DD=%.2f%%\n",
  ...
  names(port_all) <- c("Scaled_LO", "Binary_LO", "BH")
  port_df <- data.frame(Date = index(port_all), coredata(port_all))
  write.csv(port_df, sprintf("%s/portfolio-returns-lo.csv", reportPath), row.names = FALSE)
  cat(sprintf("  saved portfolio returns: portfolio-returns-lo.csv (%d rows)\n", nrow(port_df)))

  # ── gt portfolio table ──
  port_results <- tibble(
    Strategy   = c("Scaled LO (EW)", "Binary LO (EW)", "B&H (EW)"),
  ...
  port_results |>
    gt() |>
    tab_header(
      title = sprintf("Equal-Weight Portfolio (%d assets)", n_assets),
      subtitle = sprintf("From %s → %s; rebalanced monthly; drag = 0.2%%", portfolio_start,
    ...
    tab_source_note(source_note = "@StockViz") |>
    tab_style(style = cell_text(align = "right"), locations = cells_source_notes()) |>
    gtsave(sprintf("%s/portfolio-metrics-lo.html", reportPath))

  webshot2::webshot(
    sprintf("%s/portfolio-metrics-lo.html", reportPath),
    sprintf("%s/portfolio-metrics-lo.png", reportPath),
    selector = "table.gt_table", expand = c(10, 10, 10, 10))

  # ── cumulative chart: Scaled LO vs Binary LO vs B&H ──
  combined <- na.omit(merge(port_scaled, port_bin, port_bh))
  names(combined) <- c("Scaled LO (EW)", "Binary LO (EW)", "B&H (EW)")
  Common.PlotCumReturns(combined, sprintf("Equal-Weight Portfolio (%d assets)", n_assets),
    sprintf("Strategy Nine — All Assets EW from %s", portfolio_start),
    sprintf("%s/portfolio-ew-lo.cumret.png", reportPath), NULL)
  ...
  tbl <- ar_tbl |>
    gt() |>
    tab_header(
      title = sprintf("Equal-Weight Portfolio (%d assets) — Annual Returns", n_assets),
      subtitle = sprintf("Scaled LO vs Binary LO vs B&H from %s", portfolio_start)
    ) |>
  ...
  tbl |> gtsave(sprintf("%s/portfolio-annual-returns-lo.html", reportPath))
  webshot2::webshot(
    sprintf("%s/portfolio-annual-returns-lo.html", reportPath),
    sprintf("%s/portfolio-annual-returns-lo.table.png", reportPath),
    selector = "table.gt_table", expand = c(10, 10, 10, 10))
  ...
  tbl <- dd_tbl |>
    gt() |>
    tab_header(
      title = sprintf("Equal-Weight Portfolio (%d assets) — Max Drawdown", n_assets),
      subtitle = sprintf("Scaled LO vs Binary LO vs B&H from %s", portfolio_start)
    ) |>
    fmt_percent(columns = -Year, decimals = 2) |>
    tab_style(style = cell_text(weight = "bold"), locations = cells_column_labels()) |>
  ...
  tbl |> gtsave(sprintf("%s/portfolio-annual-drawdowns-lo.html", reportPath))
  webshot2::webshot(
    sprintf("%s/portfolio-annual-drawdowns-lo.html", reportPath),
    sprintf("%s/portfolio-annual-drawdowns-lo.table.png", reportPath),
    selector = "table.gt_table", expand = c(10, 10, 10, 10))
  ...
  tbl <- sr_tbl |>
    gt() |>
    tab_header(
      title = sprintf("Equal-Weight Portfolio (%d assets) — Sharpe Ratio", n_assets),
      subtitle = sprintf("Scaled LO vs Binary LO vs B&H from %s", portfolio_start)
    ) |>
  ...
  tbl |> gtsave(sprintf("%s/portfolio-annual-sharpe-lo.html", reportPath))
  webshot2::webshot(
    sprintf("%s/portfolio-annual-sharpe-lo.html", reportPath),
    sprintf("%s/portfolio-annual-sharpe-lo.table.png", reportPath),
    selector = "table.gt_table", expand = c(10, 10, 10, 10))
}
