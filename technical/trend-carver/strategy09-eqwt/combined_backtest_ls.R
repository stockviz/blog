...
if (nrow(results) > 0) {
  results |>
    ...
    gtsave(sprintf("%s/combined-summary-ls.html", reportPath))

  webshot2::webshot(
    sprintf("%s/combined-summary-ls.html", reportPath),
    sprintf("%s/combined-summary-ls.png", reportPath),
...
for (asset in names(scaled_ls)) {
  ...
  Common.PlotCumReturns(combined, asset,
    ...
    sprintf("%s/%s-ls.cumret.png", reportPath, safe_name), NULL)
}
...
if (length(valid_bin) > 1) {
  ...
  Common.PlotCumReturns(bin_all, "All Assets",
    ...
    sprintf("%s/combined-all-binary-ls.cumret.png", reportPath), NULL)
  ...
  Common.PlotCumReturns(sclo_all, "All Assets",
    ...
    sprintf("%s/combined-all-scaled-ls.cumret.png", reportPath), NULL)
}
...
for (group_name in c("NIFTY", "Crypto", "MCX", "US")) {
  ...
  Common.PlotCumReturns(g_sclo, group_name,
    ...
    sprintf("%s/group-%s.scaled-ls.cumret.png", reportPath, tolower(group_name)), NULL)
  ...
  Common.PlotCumReturns(g_bin, group_name,
    ...
    sprintf("%s/group-%s.binary-ls.cumret.png", reportPath, tolower(group_name)), NULL)
}
...
if (length(valid_bin) >= 2) {
  ...
  cat(sprintf("  Scaled LS  EW: SR=%.2f  Ret=%.2f%%  DD=%.2f%%\n",
              port_sr, port_ret*100, port_dd*100))
  cat(sprintf("  Binary LS  EW: SR=%.2f  Ret=%.2f%%  DD=%.2f%%\n",
  ...
  write.csv(port_df, sprintf("%s/portfolio-returns-ls.csv", reportPath), row.names = FALSE)
  cat(sprintf("  saved portfolio returns: portfolio-returns-ls.csv (%d rows)\n", nrow(port_df)))

  # ── gt portfolio table ──
  port_results <- tibble(
    Strategy   = c("Scaled LS (EW)", "Binary LS (EW)", "B&H (EW)"),
  ...
  port_results |>
    ...
    tab_source_note(source_note = "@StockViz") |>
    tab_style(style = cell_text(align = "right"), locations = cells_source_notes()) |>
    gtsave(sprintf("%s/portfolio-metrics-ls.html", reportPath))

  webshot2::webshot(
    sprintf("%s/portfolio-metrics-ls.html", reportPath),
    sprintf("%s/portfolio-metrics-ls.png", reportPath),
    selector = "table.gt_table", expand = c(10, 10, 10, 10))

  # ── cumulative chart: Scaled LS vs Binary LS vs B&H ──
  combined <- na.omit(merge(port_scaled, port_bin, port_bh))
  names(combined) <- c("Scaled LS (EW)", "Binary LS (EW)", "B&H (EW)")
  Common.PlotCumReturns(combined, sprintf("Equal-Weight Portfolio (%d assets)", n_assets),
    ...
    sprintf("%s/portfolio-ew-ls.cumret.png", reportPath), NULL)
  ...
  tbl |> gtsave(sprintf("%s/portfolio-annual-returns-ls.html", reportPath))
  webshot2::webshot(
    sprintf("%s/portfolio-annual-returns-ls.html", reportPath),
    sprintf("%s/portfolio-annual-returns-ls.table.png", reportPath),
    selector = "table.gt_table", expand = c(10, 10, 10, 10))
  ...
  tbl |> gtsave(sprintf("%s/portfolio-annual-drawdowns-ls.html", reportPath))
  webshot2::webshot(
    sprintf("%s/portfolio-annual-drawdowns-ls.html", reportPath),
    sprintf("%s/portfolio-annual-drawdowns-ls.table.png", reportPath),
    selector = "table.gt_table", expand = c(10, 10, 10, 10))
  ...
  tbl |> gtsave(sprintf("%s/portfolio-annual-sharpe-ls.html", reportPath))
  webshot2::webshot(
    sprintf("%s/portfolio-annual-sharpe-ls.html", reportPath),
    sprintf("%s/portfolio-annual-sharpe-ls.table.png", reportPath),
    selector = "table.gt_table", expand = c(10, 10, 10, 10))
}
