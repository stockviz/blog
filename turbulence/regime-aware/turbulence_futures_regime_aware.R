#!/usr/bin/env Rscript
# Regime-aware Bézier-Curve Market Turbulence on NIFTY and SELECT futures
# =====================================================================
...
#
# Compared systems: reversal, breakout, and reversal with rising-MA50 short filter;
# each is evaluated as long/flat and long/short.
#
# Usage:
#   Rscript turbulence_futures_regime_aware.R [--outdir PATH]
###############################################################################
...
run_system <- function(px, channel_len, threshold) {
  ...
  ma50_slope <- c(NA_real_, diff(ma50))
  ret <- c(NA_real_, v[-1L] / v[-n] - 1)

  make_position <- function(mode, filter_rising_ma = FALSE) {
    pos <- rep(0, n)
    for (i in 2:n) {
      prior <- i - 1L
      pos[i] <- pos[i - 1L]
      if (!isTRUE(turbulent[prior]) || !is.finite(v[prior])) next
      at_lower <- is.finite(lower[prior]) && v[prior] <= lower[prior]
      at_upper <- is.finite(upper[prior]) && v[prior] >= upper[prior]
      if (mode == "reversal" && at_lower) pos[i] <- 1
      if (mode == "reversal" && at_upper &&
          (!filter_rising_ma || !is.finite(ma50_slope[prior]) || ma50_slope[prior] <= 0)) {
        pos[i] <- -1
      }
      if (mode == "breakout" && at_lower) pos[i] <- -1
      if (mode == "breakout" && at_upper) pos[i] <- 1
    }
    pos
  }
  make_returns <- function(pos) {
    turnover <- abs(c(NA_real_, diff(pos)))
    list(position = pos, turnover = turnover,
         ls = pos * ret - DRAG * turnover,
         lo = pmax(pos, 0) * ret - DRAG * abs(c(NA_real_, diff(pmax(pos, 0)))))
  }
  rev <- make_returns(make_position("reversal"))
  brk <- make_returns(make_position("breakout"))
  trd <- make_returns(make_position("reversal", filter_rising_ma = TRUE))
  data.frame(date = as.Date(index(px)), price = v, upper = upper, lower = lower,
             turbulent = turbulent, ma50_slope = ma50_slope, bh = ret,
             reversal_position = rev$position, reversal_turnover = rev$turnover,
             reversal_ls = rev$ls, reversal_lo = rev$lo,
             breakout_position = brk$position, breakout_turnover = brk$turnover,
             breakout_ls = brk$ls, breakout_lo = brk$lo,
             trend_position = trd$position, trend_turnover = trd$turnover,
             trend_ls = trd$ls, trend_lo = trd$lo)
}
...
metric_row <- function(rets, instrument, system, window, selected_len, selected_thr,
  ...
  data.frame(Instrument = instrument, System = system, N = as.numeric(m[["N"]]),
             ...
             AvgExposure = mean(abs(position), na.rm = TRUE),
...
select_params <- function(px) {
  ...
  for (len in CHANNEL_GRID) for (thr in THRESH_GRID) {
    ...
    train <- bt$reversal_ls[bt$date <= TRAIN_END]
...
for (instrument in names(prices)) {
  ...
  systems <- c(
    "B&H" = "bh",
    "Reversal Long/Flat" = "reversal_lo",
    "Reversal Long/Short" = "reversal_ls",
    "Breakout Long/Flat" = "breakout_lo",
    "Breakout Long/Short" = "breakout_ls",
    "Trend-Filtered Reversal Long/Flat" = "trend_lo",
    "Trend-Filtered Reversal Long/Short" = "trend_ls"
  )
  for (window_name in c("pre", "post", "full")) {
    w <- switch(window_name,
                ...
                full = bt)
    for (system in names(systems)) {
      col <- systems[[system]]
      if (system == "B&H") {
        position <- rep(1, nrow(w)); turnover <- rep(0, nrow(w))
      } else {
        key <- if (grepl("Breakout", system)) "breakout" else if (grepl("Trend", system)) "trend" else "reversal"
        position <- w[[paste0(key, "_position")]]
        turnover <- w[[paste0(key, "_turnover")]]
      }
  ...
  daily <- xts(bt[, unname(systems)], order.by = bt$date)
  colnames(daily) <- names(systems)
  ...
  for (window_name in c("pre", "post", "full")) {
    ...
    plotCumDrawdown(as.list(daily), dateRange = range,
                    title = sprintf("Regime-Aware Market Turbulence — %s futures — %s", instrument, window_name),
                    subtitle = sprintf("Reversal vs breakout; rising-MA50 short filter; channel=%d, threshold=%.1f%%", len, 100 * thr),
