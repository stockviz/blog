...
> **Abstract:** Quantitative rotation through SPDR Sector ETFs: 5yr rolling Omega selection delivers best CAGR (12.29%). 2Y-RBL OR achieves 11.19% CAGR, beating SPY.
...
5-year rolling lookback using the "OR" criterion.
...
| Sharpe | 0.66 | 0.64 |
| MaxDD | 47.95% | 55.20% |
| Volatility | 18.81% | 19.25% |

## Files

- `annual-returns-2Y-RBL-OR.png` — Annual returns column chart
- `cumulative-2Y-RBL-OR.png` — Cumulative returns (2Y-RBL vs SPY)
- `metrics-2Y-RBL-OR.png` — Performance metrics table

## Methodology

All 11 choose 4 = 330 combinations are evaluated per 5-year rolling window.
The method ("OR") selects the best combination. The portfolio
