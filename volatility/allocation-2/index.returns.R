...
for(j in 1:ncol(allocRetXts)){
	allocSd<-merge.xts(allocSd, rollapply(allocRetXts[,j], lb, function(X) ES(X, p=.95, method="modified")))
}

sdXts1<-na.omit(rollapply(eqRets1, lb, function(X) ES(X, p=.95, method="modified")))
allocSdRatio<-NULL
...
ggplot(ratioMelt, aes(x=T, y=value, color=variable)) +
	...
	geom_line() +
	scale_x_date(breaks = xAxisTicks) +
	labs(x='', y='ES ratio', color='', title=sprintf("Ratio of Expected Shortfall for different allocations vs. %s", indexName1), subtitle=sprintf("%d week returns [%s:%s]", lb, first(index(allocSdRatio)), last(index(allocSdRatio)))) +
...
ggsave(sprintf("%s/%s.%s.ES-allocation-ratios.png", reportPath, indexName1, indexName2), width=16, height=8, units="in")
