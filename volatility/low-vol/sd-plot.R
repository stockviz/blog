...
ggplot(ratioDf, aes(x=T, y=Ratio)) +
	...
	geom_line() +
	scale_x_date(breaks = xAxisTicks) +
	labs(x='', y='sd ratio', color='', title=sprintf("%s/%s Ratio of Standard Deviation", indexName2, indexName1), subtitle=sprintf("%d week returns [%s:%s]", lb, first(index(dXts)), last(index(dXts)))) +
