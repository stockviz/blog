...
vixXts <- dailyReturn(vixXts)
vixXts <- vixXts[-1]
vixXts <- ifelse(vixXts > 0, 1, 0)

lastX <- tail(vixXts, segLength)

distXts <- rollapply(vixXts, segLength, function(X) sum(coredata(X) != coredata(lastX)), by.column=F)
...
tt1<-arrangeGrob(tableGrob(smlDist, rows=NULL, theme=tableTheme), ncol=1, top=textGrob("India VIX Hamming Distance", gp=gpar(fontsize=12, fontface ='bold', fontfamily='Segoe UI')), bottom=textGrob("@StockViz", gp=gpar(fontsize=10, col='grey', fontfamily='Segoe UI')))
...
ggsave(sprintf("%s/closest-match-hamming.png", reportPath), width=16, height=14, units="in")